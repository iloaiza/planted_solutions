...
def cartan_orbtransf(tensor, U, complex=True):
    ...
    '''
    if not complex:
        return np.einsum('ak,bl,cm,dn,klmn->abcd', U, U, U, U, tensor)
#         al,bl,cm,dm,llmm->abcd
    else:
        return np.einsum('ak,bl,cm,dn,klmn->abcd', U, np.conj(U), U, np.conj(U), tensor)
#         return np.einsum('al,bl,cm,dm,llmm->abcd', U, np.conj(U), U, np.conj(U), tensor)


def rotated_cartan(ctvals, U, n, k, complex):
    '''
...
def get_w_o(lmbdas, o_params, n, k):
    ...
    '''
#     def get_cartan_matrix(lmbdas, k, n):
#         '''
...
def csa(target, k, alpha=1, tol=1e-6, grad=False, complex=False, x0=None, maxiter=10000, disp=False):
    '''
