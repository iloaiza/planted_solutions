"""Obtain the VCSA fragments FCI variances. 
This also checks that all fragments sums up to the Hamiltonian as expected. 

Usage: python vcsa_variance_compute.py (mol) (csa_nfrag) 
"""
import sys
sys.path.append("../")
import numpy as np
...
# Prep
vcsa_sol = sl.load_csa_sols(mol, geo, method, csa_nfrag)
vcsa_fragments = vcsa_sol['grouping']
vcsa_tbts = csau.get_tbt_parts(vcsa_sol['sol_array'],
                               vcsa_sol['n_qubits'] // 2, vcsa_sol['csa_alpha'], complex=False)
...
tbtev_ln = sl.load_tbt_variance("ev", mol, geo=1.0, wfs_type='fci', verbose=False)
tbtev_sq = sl.load_tbt_variance("sq", mol, geo=1.0, wfs_type='fci', verbose=False)

# Compute FCI variance
group_vars = np.full(len(vcsa_fragments), np.nan)
for idx, frag in enumerate(vcsa_fragments):
    if idx == 0:
        curvar = of.variance(of.get_sparse_operator(frag, n_qubits), psi)
        group_vars[idx] = np.real_if_close(curvar, tol=1e4)
    else:
        group_vars[idx] = csau.get_precomputed_variance(vcsa_tbts[idx-1], tbtev_ln, tbtev_sq)
...
if save:
    sl.save_variance_result(mol, wfs=wfs, method=method.lower(), 
                            geometry=geo, groups=vcsa_fragments, variances=group_vars)
